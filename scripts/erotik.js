    ...
    try {
        ...
        while (true) {
            ...
            const hrefUrls = await page.evaluate(() => {
                ...
                const links = document.querySelectorAll('a.clsy-c-result-list-item__link[href]');
            ...
            for (const url of hrefUrls) {
                try {
                    const [rows] = await connection.execute('SELECT COUNT(*) AS count FROM erotik_urls WHERE url = ?', [url]);
                    const urlExists = rows[0].count > 0;

                    if (!urlExists) {
                        await connection.execute('INSERT INTO erotik_urls (url) VALUES (?)', [url]);
...
// Example usage
const baseUrl = 'https://erotik.markt.de/anzeigen/sie-sucht-ihn/'; // Replace with your desired base URL
