                        ...
                        <table class="table table-responsive-sm" id="campaigns-table">
                            ...
                            @foreach($m as $c)
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <a href="/campaign/delete/{{$c->id}}"
                                               class="btn btn-danger shadow btn-xs sharp"><i
                                                    class="fa fa-trash"></i></a>
                                            <a href="/campaign/edit/{{$c->id}}"
                                               class="btn btn-info shadow btn-xs sharp"><i class="fa fa-edit"></i></a>
                                        </div>
                                    </td>
                                    @if($c->status == 1)
                                        <td>
                                            @if($c->sending_status === 0)
                                                <a href="/start_sending/{{$c->id}}" class="btn btn-primary">Start</a>
                                            @else
                                                <a href="/stop_sending/{{$c->id}}" class="btn btn-danger">Stop</a>
                                            @endif
                                        </td>
                                    @endif
                                </tr>
